...
def _prefer_downsort(kicks: List[Tuple[int,int]]) -> List[Tuple[int,int]]:
    return sorted(kicks, key=lambda d: (-d[1], abs(d[0])))
...
def rotate_srs(
    ...
    pos = tuple(piece["pos"])
    rot = piece["rotation"]
    new_rot, new_pos, ok = try_rotate_srs(name, pos, rot, direction, board, collides_fn, prefer_down)
    if ok:
